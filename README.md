...
`needs_review`는 기존 DB의 과거 검토 상태로 남을 수 있습니다. 새 수집에서는 Tier 1/2와 제목상 기후 관련성이 확인되는 Tier 3를 저장합니다. 메일·Excel은 Tier 1/2만 다루고, Tier 3는 대시보드의 참고 목록에만 표시합니다.
...
| Tier 3 | 기후·온실가스·배출권·외부사업 관련성이 있지만 직접 컨설팅·고객사 신청형 지원은 아닌 물품 구매·연구·행사 등 | 대시보드 참고 목록만 표시, 메일 제외 |

선정 기업이 지원사업 예산으로 진행하는 압축기·모터 등의 구매입찰은 지원사업 **신청 기회가 아니므로** Tier 2가 아니라 Tier 3 참고 공고로 표시합니다. 단순 `환경` 시설공사, 발주기관명에만 기후 용어가 있는 건, 교복 등 무관한 공고는 대시보드에서도 제외합니다. 기존 기록은 삭제하지 않고, 샘플 공고는 운영 출력에서 제외합니다. 자동 분류는 최종 입찰 적합성 판단이 아닙니다.
...
- 용역·물품·공사·외자 4개 공식 검색 API에서 최근 공고를 반복 수집하고, 별도 일일 조회는 공고명 키워드 제한 없이 현재 접수 중 후보를 확인한 뒤 Tier 1/2와 기후 관련 Tier 3 공고를 새로 저장합니다.
- 전체 조회는 최근 90일을 API 허용 범위인 최대 30일 구간으로 나눠 조회하고, `totalCount`에 맞춰 페이지를 끝까지 읽습니다. 입찰 시작 전 공고와 마감이 지난 공고는 제외합니다.
- 일일 전체 조회는 최대 500 API 요청, 구간별 최대 100페이지로 안전 제한합니다. 후보 중 입찰 시작 전·마감 후 공고는 제외하고, 제목상 기후 관련성이 확인되는 공고만 DB에 새로 저장합니다.
...
- **추가 고객사 기후지원 후보**: [한국환경산업기술원 공지·공고](https://www.keiti.re.kr/site/keiti/ex/board/List.do?cbIdx=277)는 로컬 정기 수집에 연결했습니다. 최근 2쪽 제한 조회에서 적합 신규 공고 0건이며, 기한이 확인되지 않으면 진행 중으로 표시하지 않습니다. [한국산업단지공단 탄소중립 융자](https://www.kicox.or.kr/netzerofin/pbanc/pbancList.do)는 현재 Python HTTPS 오류로 비활성화했습니다. [기업마당 공식 API](https://www.bizinfo.go.kr/apiDetail.do?id=bizinfoApi)는 연결기와 모의응답 테스트만 완료했고, 별도 `BIZINFO_API_KEY` 및 외부 공개 조건 확인 전에는 비활성화합니다. 나라장터 서비스키는 기업마당 인증키가 아닙니다.
